...
class Upload extends Component{
    ...
    public function upload($files='') {
        ...
        foreach ($files as $key => $file) {
            ...
            if(false === $subpath){
                ...
                $file['save_path'] = $this->savePath . $subpath;
            }

            /* 对图像文件进行严格检测 */
            $ext = strtolower($file['ext']);
            if(in_array($ext, array('gif','jpg','jpeg','png','swf'))) {
    ...
    private function setDriver($driver = null, $config = null){
        ...
        $upload_config = $this->getDi()->get('config')->upload->toArray();
        $driver = $driver ? : ($this->driver       ? : $upload_config['default']);
        $config = $config ? : ($this->driverConfig ? : $upload_config['drivers'][$driver]['driverConfig']);
        $class = strpos($driver,'\\')? $driver : '\\Component\\Upload\\Driver\\'.ucfirst(strtolower($driver));
